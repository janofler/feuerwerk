...
class PygView(object):
    ...
    def paint(self):
        ...
        self.ball3 = Ball(pos = v.Vec2d(180,500), move = v.Vec2d(0,0), bounce_on_edge = True, upkey = pygame.K_i, downkey = pygame.K_k, leftkey = pygame.K_j, rightkey = pygame.K_l, mass = 1000)
        self.cannonA = Cannon(pos = (10,10), color = (120,120,120))
        self.cannonB = Cannon(pos = (PygView.width-30,10))
        self.cannonC = Cannon(pos = (10,PygView.height-10))
        self.cannonD = Cannon(pos = (PygView.width-30, PygView.height-10))  
    ...
    def run(self):
        ...
        while running:
            for event in pygame.event.get():
                ...
                # ------- pressed and released key ------
                    ...
                    if event.key == pygame.K_b:
                        Ball(pos=v.Vec2d(self.ball1.pos.x,self.ball1.pos.y), move=v.Vec2d(0,0), radius=5, friction=0.9, bounce_on_edge=True) # add small balls!
                    if event.key == pygame.K_n:
                        Ball(pos = v.Vec2d(random.randint(0,PygView.width),random.randint(0,PygView.height)),color = (85,47,130), move = v.Vec2d(0,0), radius=20, friction = 0.999, bounce_on_edge = True)
            ...
            #----------cannonB auf ball1-----------------
            vectordiff = self.cannonB.pos - self.ball1.pos
            self.cannonB.set_angle(-vectordiff.get_angle()-180)
            
            
            #------------cannonC auf ball2--------------
            vectordiff = self.cannonC.pos - self.ball2.pos
            self.cannonC.set_angle(-vectordiff.get_angle()-180)
            
            #----------cannonD auf ball2----------------
            vectordiff = self.cannonD.pos - self.ball2.pos
            self.cannonD.set_angle(-vectordiff.get_angle()-180)
            ...
            if random.random() < 0.01:
                ...
                #print(p, m)
                Ball(pos=p, move=m.normalized()*15, radius=10, mass= 500, color=(100,100,
                random.randint(000,150)))
